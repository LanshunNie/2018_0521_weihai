...
#if MLCDEBUG
  printf("mlc update_by_new_schedule:");
#endif
  int temp = 0;
  for(;temp<sizeof(schedule_bitmap);temp++){
    printf("%x ",schedule_bitmap_new[temp]);
  }
  printf("\n");
}
...
#if MLCDEBUG
          printf("mlc invalid netsyn_offset and return\n");
#endif 
...
#if MLCDEBUG
              printf("mlc get_idle_time < 60 and give up\n");
#endif 
            }
          }else{
            netsync_init_flag =1;
            // void* temp1;  //update by mlc for schedule_bitmap_init()
            update_new_schedule(NULL);  //update immediately
            // ctimer_set(&send_netsync_ct,15*CLOCK_SECOND,netsynch_send,NULL);
...
netsynch_send(void * p)
{
  ...
  if(get_idle_time() > 4){
    ...
    #if MLCDEBUG
    printf("mlc not syned and not send\n");
#endif 
  } 
}
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
void request_to_pan(void *p){
 // uip_ipaddr_t ipaddr;
  #if MLCDEBUG
    printf("mlc request to pan\n");
#endif 
    struct netsynch_msg msg;
    msg.seqnum = (last_seqnum);
    msg.authority_level =( AUTHORITY_LEVEL_UNSYNCHED);
