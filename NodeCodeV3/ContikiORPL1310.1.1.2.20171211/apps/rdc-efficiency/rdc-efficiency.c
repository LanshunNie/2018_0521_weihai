...
PROCESS_THREAD(rdc_efficiency_process, ev, data)
{
  ...
  while(1) {
    ...
    {
      extern uint8_t for_efficiency_test;
      energy_efficiency_node_behaviour_check();

      if(energy_efficient_flag == 1 && for_efficiency_test > 0)
      {
...
rdc_efficiency_init(void)
{
  ...
  ENERGY_EFFICIENCY_INIT(25000,RTIMER_ARCH_SECOND/get_rdc_active_channel_check_rate(),10000);
  // ENERGY_EFFICIENCY_INIT(145000,RTIMER_ARCH_SECOND/get_rdc_active_channel_check_rate(),10000);
