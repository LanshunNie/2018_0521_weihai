...
static uint8_t low_latency_one_second_before =0;
...
set_lowLatency_flag(soft_time  timenow){
  ...
  if(get_active_flag() == 0){
    ...
    if(index != previous_index){
      ...
      #endif
      // if(low_latency_active_second > 0){
      //   low_latency_falg = 1;
      // }
      // ctimer_set(&auto_sleep_ctimer , low_latency_active_second * 4 / 5 * CLOCK_SECOND,auto_sleep_callback,NULL);
    }
    // printf("bbbbbbbbbbbbbb %d %d %d\n",index_change_flag,timenow.sec,low_latency_active_second);
...
void set_low_latency_one_second_before(uint8_t low_one_before){
  low_latency_one_second_before = low_one_before;
}
/*---------------------------------------------------------------------------*/
uint8_t get_low_latency_one_second_before(void){
  return low_latency_one_second_before;
}
